...
- **No RTMP/HLS ingest or CDN.** Live video is real, peer-to-peer WebRTC between the streamer's camera and each viewer (signaled over Socket.io) — great for a demo, but it doesn't scale to large audiences the way a real broadcast pipeline (RTMP ingest + HLS/CDN delivery) would. That's a dedicated media-server infrastructure project on its own.
